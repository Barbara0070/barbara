...
def oferta():
    for i in slownik:
        print(f"{i} cena:{slownik[i]} ilosc na stanie: {stan[i]}")
oferta()
def dodaj(towar, sztuki, cena):
        slownik[towar]=cena
        stan[nazwa]+=stan.get(nazwa, 0)
def zakup(towar, sztuki):
    if stan[towar]>=sztuki:
        stan.towar-=sztuki
        return slownik[towar]*sztuki
    if stan[towar]<sztuki:
        return False
while(True):
    print("oferta - o\n dodaj - d\n zakup -z")
